...
A human-readable Markdown report generated after org-wide quality profile migration. For each language, it renders a side-by-side table of:
- **Missing from SonarCloud** — Rules active in SonarQube that are absent on SonarCloud (issues that will disappear after migration)
- **Added in SonarCloud** — Rules present on SonarCloud but not in SonarQube (new issues that may appear after migration)

Generated by `src/shared/reports/format-rules-comparison/index.js` using the `rulesComparisonData` attached during `migrate-org-wide-batch2.js`. Written to the output directory by `write-text-reports.js`.

<!-- <subsection-updated last-updated="2026-04-02T00:00:00Z" updated-by="Claude" /> -->
### Project Issues Delta Report (`issues-delta-report.md`)

A per-project Markdown report comparing actual live issues on SonarQube against actual live issues on SonarCloud post-migration. For each project and each rule, it reports:
- **Disappeared issues** — Issues present in SonarQube but absent from SonarCloud (count per rule)
- **Appeared issues** — Issues present in SonarCloud but absent from SonarQube (count per rule)

The data is gathered by `src/pipelines/sq-2025/sonarcloud/reports/issues-delta/` (`gatherAllDelta` → `gatherProjectDelta` → `diffProjectIssues` + `buildRuleBreakdown`) and rendered by `src/shared/reports/format-issues-delta/index.js`. The migrate pipeline calls `gatherIssuesDelta` immediately before writing reports, so the delta always reflects the post-migration state.
