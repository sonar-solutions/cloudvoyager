...
One row per branch per project. The main branch is listed first for each project.
...
**Example:**
```csv
Include,Project Key,Project Name,Branch,Target Organization,ALM Platform,Repository,Monorepo,Visibility,Last Analysis
yes,my-project,My Project,main,my-org,github,my-org/my-repo,false,public,2026-02-20
yes,my-project,My Project,develop,my-org,github,my-org/my-repo,false,public,2026-02-20
no,my-project,My Project,feature/old,my-org,github,my-org/my-repo,false,public,2026-02-20
```

In this example, `main` and `develop` branches will be migrated. The `feature/old` branch is excluded.

**Backward compatibility:** If your CSV was generated by an older version and does not have a Branch column, all branches will be included (same as before).
...
In `projects.csv`, set `Include` to `no` for **all** branch rows of the project (or at minimum the main branch row).

### Exclude specific branches from migration
In `projects.csv`, set `Include` to `no` for individual branch rows you want to skip. The main branch must remain `yes` for the project to be migrated.
