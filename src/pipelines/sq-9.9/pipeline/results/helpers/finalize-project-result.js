// -------- Finalize Project Migration Result Status --------
...
export function finalizeProjectResult(projectResult) {
  ...
  if (failedSteps.length === 0) projectResult.status = 'success';
  else if (failedSteps.length === nonSkippedSteps.length) projectResult.status = 'failed';
  else projectResult.status = 'partial';
