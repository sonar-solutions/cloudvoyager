...
// -------- Extract New Code Period Definitions --------
...
export async function extractNewCodePeriods(client, projectKey = null) {
  ...
  if (projectLevel) {
    ...
    result.projectLevel = {
      type: projectLevel.type, value: projectLevel.value || null,
  ...
  for (const branch of branchOverrides) {
    ...
    result.branchOverrides.push({
      branchKey: branch.branchKey, type: branch.type,
      value: branch.value || null, settings: mapper ? mapper(branch) : null,
    });
  }
