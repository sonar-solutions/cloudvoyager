...
// -------- Extract Quality Gates with Conditions and Permissions --------
...
export async function extractQualityGates(client) {
  ...
  const detailed = await mapConcurrent(gates, async (gate) => {
    ...
    return {
      name: gate.name, isDefault: gate.isDefault || false, isBuiltIn: gate.isBuiltIn || false,
      conditions: (details.conditions || []).map(c => ({ id: c.id, metric: c.metric, op: c.op, error: c.error })),
      permissions
    };
  }, { concurrency: 5 });
