...
// -------- Set Quality Profile Permissions --------

export async function setProfilePermissions(customProfiles, restored, client) {
  for (const profile of customProfiles) {
    if (!restored.has(profile.key)) continue;
    await setGroupPermissions(profile, client);
    await setUserPermissions(profile, client);
  }
}

async function setGroupPermissions(profile, client) {
...
async function setUserPermissions(profile, client) {
