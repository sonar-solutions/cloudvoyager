...
// -------- SonarQube API Error Handler --------

export function handleError(error, baseURL) {
  if (error.response) {
  ...
  if (error.request) {
    return handleRequestError(error, baseURL);
  }
  throw new SonarQubeAPIError(`Request failed: ${error.message}`);
}

function handleRequestError(error, baseURL) {
  const base = baseURL || error.config?.baseURL || 'unknown';
  const code = error.code || 'UNKNOWN';
  let msg = `Cannot connect to SonarQube server at ${base}`;
