...
export class SonarQubeClient {
  ...
  async getMeasures(branch = null, metricKeys = []) {
    ...
    // SonarQube 9.x limits metricKeys to 15 per request — batch if needed
    ...
    if (metricKeys.length > MAX_METRIC_KEYS) {
      ...
      for (let i = 0; i < metricKeys.length; i += MAX_METRIC_KEYS) {
        ...
        mergedMeasures = mergedMeasures.concat(component.measures || []);
      }
      return { key: this.projectKey, measures: mergedMeasures };
    }
  ...
  async getComponentTree(branch = null, metricKeys = []) {
    ...
    // SonarQube 9.x limits metricKeys to 15 per request — batch and merge if needed
    const MAX_METRIC_KEYS = 15;
    if (metricKeys.length > MAX_METRIC_KEYS) {
