...
export async function extractActiveRules(client, components = []) {
  ...
  try {
    ...
    if (profiles.length === 0) { logger.warn('No quality profiles found for project'); return []; }
    ...
    const repoCounts = {};
    allActiveRules.forEach(r => { repoCounts[r.ruleRepository] = (repoCounts[r.ruleRepository] || 0) + 1; });
    logger.info('Active rules breakdown by repository:');
    Object.entries(repoCounts).forEach(([repo, count]) => logger.info(`  ${repo}: ${count}`));
