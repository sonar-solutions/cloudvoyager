...
// -------- Main Phases Part 1: Metadata through Hotspots --------

export function buildPhasesPart1(extractor, ctx) {
  return [
    { name: 'extract:project_metadata', label: 'Step 1: Extracting project data',
      fn: async () => {
        ctx.project = await extractProjectData(extractor.client);
        const rev = await extractor.client.getLatestAnalysisRevision();
        if (rev) ctx.scmRevisionId = rev;
