...
// -------- Main Phases Part 2: Measures through Syntax Highlighting --------

export function buildPhasesPart2(extractor, ctx, opts) {
  return [
    ...
    { name: 'extract:sources', label: 'Step 7: Extracting source code',
      fn: async () => { ctx.sources = await extractSources(extractor.client, null, opts.maxFiles, { concurrency: opts.sourceConcurrency }); return ctx.sources; },
