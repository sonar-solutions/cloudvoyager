...
export function resolvePortfolioProjects(portfolio, projectKeyMapping, projectUuidMap) {
  ...
  for (const project of portfolio.projects) {
    ...
    if (uuids) resolvedProjects.push({ id: uuids.id, branchId: uuids.branchId });
    else logger.debug(`Portfolio "${portfolio.name}": project "${scProjectKey}" not found in SonarCloud — skipping`);
  }
