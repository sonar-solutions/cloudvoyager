...
// Build quality profile mappings per language for scanner report metadata.
export function buildQProfiles(instance) {
  const qprofiles = {};
  const scLanguages = new Set(instance.sonarCloudProfiles.map(p => p.language.toLowerCase()));
  const languages = [...new Set([
    ...instance.data.activeRules.map(r => r.language).filter(Boolean),
    ...instance.data.sources.map(s => s.language).filter(Boolean)
  ])];

  languages.forEach(language => {
    const languageKey = language.toLowerCase();
    const scProfile = instance.sonarCloudProfiles.find(p => p.language === languageKey);
    if (scProfile) {
