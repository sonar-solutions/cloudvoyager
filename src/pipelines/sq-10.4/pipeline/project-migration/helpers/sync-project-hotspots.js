...
/**
 * Sync hotspot metadata from SonarQube to SonarCloud for a single project.
 */
export async function syncProjectHotspots(projectResult, results, reportUploadOk, ctx, scProjectKey, projectSqClient, projectScClient, projectKey) {
  ...
  try {
    ...
    const stats = await syncHotspots(scProjectKey, sqHotspots, projectScClient, { concurrency: ctx.perfConfig.hotspotSync.concurrency, sonarqubeUrl: projectSqClient.baseURL, sonarqubeProjectKey: projectSqClient.projectKey, logPrefix: `[${projectKey}]` });
    ...
    projectResult.steps.push({ step: 'Sync hotspots', status: 'success', detail: `${stats.matched} matched, ${stats.statusChanged} status changed`, durationMs: Date.now() - start });
  } catch (error) {
