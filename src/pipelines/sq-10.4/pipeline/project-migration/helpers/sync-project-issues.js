...
/**
 * Sync issue metadata from SonarQube to SonarCloud for a single project.
 */
export async function syncProjectIssues(projectResult, results, reportUploadOk, ctx, scProjectKey, projectSqClient, projectScClient, projectKey) {
  ...
  try {
    ...
    const stats = await syncIssues(scProjectKey, sqIssues, projectScClient, { concurrency: ctx.perfConfig.issueSync.concurrency, sqClient: projectSqClient, userMappings: ctx.userMappings, logPrefix: `[${projectKey}]` });
