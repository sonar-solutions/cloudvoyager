...
/** Load existing CSV mappings and cached server-wide data from previous runs. */
export async function loadPreExistingData(outputDir, dryRun) {
  ...
  if (!dryRun && existsSync(mappingsDir)) {
    try {
      ...
      if (e.code === 'ENOENT') { preExistingCsvs = null; }
      else {
  ...
  if (!dryRun && existsSync(cacheFile)) {
    ...
    } catch (e) { logger.warn(`Failed to read cache: ${e.message}. Will re-extract.`); }
  }
