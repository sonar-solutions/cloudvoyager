...
/** Handle the scanner report upload step with journal checkpointing. */
export async function handleScanDataUpload(project, scProjectKey, org, projectResult, projectScClient, ctx, shouldRun, only, isStepDone, recordStep) {
  ...
  if (only) {
    ...
    logger.error(`Project "${scProjectKey}" does not exist in SonarCloud. Skipping all per-project steps.`);
