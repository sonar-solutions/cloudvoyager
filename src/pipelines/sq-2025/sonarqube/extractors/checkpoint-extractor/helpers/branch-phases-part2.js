...
// -------- Branch Phases Part 2 (measures, sources, dups, scm, symbols, highlighting) --------

/** Build the second group of branch extraction phases. */
export function buildBranchPhasesPart2(extractor, ctx, branch, maxFiles, sourceConcurrency, dupConcurrency) {
  return [
    { name: 'extract:measures', label: `[${branch}] Extracting project measures`,
      fn: async () => { ctx.measures = await extractMeasures(extractor.client, ctx.metricKeys, branch); return ctx.measures; },
