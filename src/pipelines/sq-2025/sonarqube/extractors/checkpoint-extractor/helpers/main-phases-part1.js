...
// -------- Main Phases Part 1 (Steps 1-5b) --------

/** Build phases 1 through 5b for main branch extraction. */
export function buildMainPhasesPart1(extractor, ctx) {
  return [
    { name: 'extract:project_metadata', label: 'Step 1: Extracting project data',
      fn: async () => { ctx.project = await extractProjectData(extractor.client); const rev = await extractor.client.getLatestAnalysisRevision(); if (rev) ctx.scmRevisionId = rev; return { project: ctx.project, scmRevisionId: ctx.scmRevisionId }; },
