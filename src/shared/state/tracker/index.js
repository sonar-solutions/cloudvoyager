...
function createStateTracker(stateFilePath) {
  ...
  const self = {
    ...
    async save() { await storage.save(state); },
    async saveAfterBranch(name) { self.markBranchCompleted(name); await self.save(); logger.debug(`State saved after branch completion: ${name}`); },
    ...
    async recordTransfer(stats) { self.updateLastSync(); self.addSyncHistory({ success: true, stats }); await self.save(); logger.info('Transfer recorded in state'); },
  };
  return self;
}
